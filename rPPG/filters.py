...
@jit(["int32[:](float32[:,:,:], int32, int32)", "int32[:](float64[:,:,:], int32, int32)",
      "int32[:](int32[:,:,:], int32, int32)"], nopython=True, nogil=True, parallel=True, fastmath=True)
...
def rgb_filter_th(sig, **kargs):
    ...
    if np.sum(goodidx) == 0:
        raise Exception(
            f"Too tight thresholds {(kargs['RGB_LOW_TH'], kargs['RGB_HIGH_TH'])} for pixel colors, blank face mask!")
